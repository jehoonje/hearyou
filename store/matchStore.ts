...
export const useMatchStore = create<MatchState>((set, get) => ({
  ...
  fetchCurrentMatch: async (currentUser: User | null, isCalledFromSubscription = false) => {
    ...
    try {
      ...
      if (rpcData && Array.isArray(rpcData) && rpcData.length > 0) {
        ...
        if (typeof fetchedPartnerId === 'string' && fetchedPartnerId) {
          ...
          if (partnerProfile?.username) {
            ...
            console.warn(`### (RPC) Partner profile or username not found for ${fetchedPartnerId}. Match considered incomplete.`);
          }
        } else {
          console.error('### (RPC) Found match data but partner ID is invalid:', matchData);
          set({ error: '매치 데이터 오류 (파트너 ID 이상)' });
        }
      ...
      // 전체 로직 에러 처리
      console.error("### (RPC) Error fetching current match:", err);
