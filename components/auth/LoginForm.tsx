...
const LoginForm = memo<LoginFormProps>(
  ({
    ...
    const getDynamicTitleVariants = () => {
      if (isKeyboardOpen && animationStage === "formVisible") {
        return {
          ...titleVariants,
          formVisible: {
            y: "-10vh",
    ...
    return (
      <div
        ...
        style={{
          paddingTop: isKeyboardOpen && animationStage === "formVisible" 
            ? `${containerPadding.top}px` 
            : isNativeApp ? '64px' : '96px',
          paddingBottom: isKeyboardOpen ? `${containerPadding.bottom}px` : '0',
