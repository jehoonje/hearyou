...
const containerVariants = {
  hidden: { opacity: 0 },
  visible: { opacity: 1, y: -100, transition: { staggerChildren: 0.1 } },
  exit: {
    opacity: 0,
...
const titleVariants = {
  ...
  formVisible: { y: -20, transition: { type: "spring", stiffness: 100 } }, // 폼 표시될 때 타이틀 위치 조정
};
      ...
      <div className="w-full max-w-md mx-auto p-2 flex flex-col items-center justify-start h-full pt-16 md:pt-24">
        ...
        {animationStage === "buttonsVisible" && (
          <motion.div
            ...
            animate={{ opacity: 1 }} // 애니메이션 목표 상태: 불투명
          ...
          <AnimatePresence mode="wait">
            ...
            {animationStage === "buttonsVisible" && (
              ...
              >
                ...
                >
                  {/* 버튼 컨테이너 (새 스타일 적용) */}
                  <div className="relative p-[12px_24px]">
                    {" "}
                    {/* :before 스타일 위한 사용자 정의 클래스 */}
                    {/* 버튼 노이즈 SVG (스타일 적용) */}
                    <svg
                      viewBox="0 0 100 100"
              ...
              >
                {authView === "login" ? (
                  <motion.form onSubmit={handleLogin} className="space-y-4">
                    <motion.div
                      variants={itemVariants} // 개별 아이템 애니메이션 적용 (여기서 formVisible 상태 사용)
                    >
                      <label
                        htmlFor="email"
                    ...
                    >
                      <button
                        type="button"
                        onClick={() => handleSetAuthView("signup")}
                  ...
                  <motion.form onSubmit={handleSignUp} className="space-y-4">
                    ...
                    >
                      <button
                        type="button"
                        onClick={() => handleSetAuthView("login")}
                        className="text-gray-400 hover:text-gray-300 font-mono text-sm"
                      >
                        이미 계정이 있으신가요? 로그인
                      </button>
